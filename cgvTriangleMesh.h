...
{  protected:
    // Attributes
    long int num_vertex = 0; ///< Number of vertices in the triangle mesh
    float *vertex = nullptr; ///< Array with the (num_vertices * 3) coordinates of the vertices
    float *normals = nullptr; ///< Array with the (num_vertices * 3) coordinates of the normal at each vertex (only for sphere generation)
...
public:
    ...
    void changeNormal(){normal = !normal;}
    void changeShader() {gouraudShading = !gouraudShading;}

};

#endif   // __CGV_TRIANGLEMESH
